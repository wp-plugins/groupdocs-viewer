...
function grpdocs_getdocument($atts) {
	...
	if (isset($protocol) && $protocol == 'https') {
		$code = "<iframe src='https://apps.groupdocs.com/document-viewer/embed/{$guid}?&referer=wordpress/1.3.10' frameborder='0' width='{$width}' height='{$height}'></iframe>";
	} 
	else {
		$code = "<iframe src='http://apps.groupdocs.com/document-viewer/embed/{$guid}?&referer=wordpress/1.3.10' frameborder='0' width='{$width}' height='{$height}'></iframe>";
	}
